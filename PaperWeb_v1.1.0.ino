    ...
    inflateEnd(&strm);
            ...
            while (http.connected() && millis() - startTime < timeoutMs) {
                ...
                if (stream->available()) {
                    ...
                    if (progressCallback && bytesDownloaded % 1024 == 0) progressCallback(bytesDownloaded, totalBytes);
                    processHTMLChar(c);
                } else {
                    noDataCount++;
                    if (noDataCount > 500) break;
...
void drawUI() {
    ...
    if (isReading) {
        ...
        canvas.printf("[ENT] Search/URL | [L] Books");
    }
...
void setup() {
    ...
    canvas.drawCenterString("SMART WEB & BOOKMARKS", 120, 115); 
  ...
  if (ssids[0] != "") {
    WiFi.begin(ssids[0].c_str(), passs[0].c_str());
  }
