...
public:
    ...
    bool fetch(String url, int timeoutMs = 60000, void (*progressCallback)(int, int) = nullptr) {
        ...
        http.addHeader("User-Agent", "PaperWeb/0.5");
...
void onLineReady(String line, int lineNum) {
    ...
    if (lineNum < 100) {
        ...
        // Вычисляем высоту текста как в v0.4
...
void drawUI() {
    ...
    // Формируем текст для отображения (как в v0.4)
    String textToShow = "";
...
void loadWebPage() {
    ...
    bool ok = engine.fetch(currentURL, 60000, updateProgress); // ← убрал 8000
...
void setup() {
    ...
    canvas.drawCenterString("STREAMING RENDERER", 120, 115); 
