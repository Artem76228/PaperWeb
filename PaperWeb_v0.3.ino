...
public:
    ...
    bool fetch(String url, int maxChars = 12000) {
        ...
        while (http.connected() && filteredText.length() < maxChars) {
            if (ESP.getFreeHeap() < 30000) break; // Защита от вылета по памяти
            ...
            if (stream->available()) {
                ...
                if (c == '>') { 
                    ...
                    if (!isSkipping && (currentTag == "p" || currentTag == "div" || currentTag == "br")) filteredText += "\n";
                    continue;
                }
...
void drawUI() {
    ...
    if (!isReading) {
        if (WiFi.status() == WL_CONNECTED) pageContent = "WELCOME TO PAPERWEB v0.3\n[ENT] - SEARCH | [TAB] - WIFI";
...
void setup() {
    ...
    // --- РИСУЕМ ЛОГОТИП НА ЗАПУСКЕ ---
    canvas.fillSprite(BLACK);
    canvas.setTextColor(WHITE);
    canvas.setTextSize(2);
    canvas.drawCenterString(APP_NAME, 120, 30); // "PaperWeb"
    canvas.drawLine(40, 65, 200, 65, CYAN);
    
    canvas.setTextSize(1);
    canvas.setTextColor(CYAN);
    canvas.drawCenterString("v" + APP_VERSION, 120, 75); // "v0.3"
    
    canvas.setTextColor(0xBDD7); // Светло-серый
    canvas.drawCenterString("by " + AUTHOR, 120, 95); // Твой ник
    
    // --- ВОТ ТВОЯ НОВАЯ СТРОЧКА ---
    canvas.setTextColor(ORANGE); // Оранжевый, чтобы выделялось
...
void loop() {
    if (millis() > targetTime) {
        ...
        auto status = M5Cardputer.Keyboard.keysState();
        if (isReading) {
            if (M5Cardputer.Keyboard.isKeyPressed(';')) { if (scrollPos > 0) scrollPos -= 4; }
            if (M5Cardputer.Keyboard.isKeyPressed('.')) { scrollPos += 4; }
